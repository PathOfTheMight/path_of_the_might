...
execute if score #Mult Number matches 0.. as @e[tag=Fireball,distance=..2,tag=New] at @s run tp @s ~ ~ ~ ~9 ~

tag @e[tag=Fireball,tag=NewMultLast,distance=..2,limit=1] remove NewMultLast
execute if score #Mult Number matches 0.. run tag @e[tag=Fireball,tag=NewMult,distance=..2,limit=1] add NewMultLast
tag @e[tag=Fireball,tag=NewMult,distance=..2,limit=1] remove NewMult

execute if score #Mult Number matches -1 as @e[tag=Fireball,distance=..2,tag=New] run function skill:act/fireball/act2
execute if score #Mult Number matches 0.. run function skill:act/fireball/act1
